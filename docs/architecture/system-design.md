...
```mermaid
graph TB
    subgraph Client [Client Tier - AWS Amplify / Edge]
    ...
    subgraph Persistence [Data Tier - Supabase Managed Instance]
...
sequenceDiagram
    ...
    alt Ratio <= 5% (Comprehensibility Verified)
    ...
    else Ratio > 5% (Lexical Drift Detected)
